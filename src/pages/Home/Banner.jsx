...
export default function Banner() {
    ...
    useEffect(() => {
        async function getCount() {
            const countData = await apiRequest(`http://127.0.0.1/offerCount`, "GET");
    ...
    return (
        <div className="position-relative d-flex flex-column justify-content-center">
            ...
            <div className="position-absolute top-50 start-50 w-100 translate-middle d-flex flex-column align-items-center gap-4">
                ...
                <h1 className="fs-1 d-lg-block d-none m-0">{`${count} sprawdzonych ofert pracy`}</h1>
                    <h2 className="fs-2 d-md-block d-lg-none d-none m-0">{`${count} sprawdzonych ofert pracy`}</h2>
                    <h3 className="fs-4 d-sm-block d-md-none d-none m-0">{`${count} sprawdzonych ofert pracy`}</h3>
                    <h4 className="fs-6 d-block d-sm-none m-0">{`${count} sprawdzonych ofert pracy`}</h4>
